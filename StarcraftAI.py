...
class Noumaan(sc2.BotAI):
    def __init__(self, use_model=False, title=1):
        ...
        if self.use_model:
            print("USING MODEL!")
            self.model = keras.models.load_model("BasicCNN-30-epochs-0.0001-LR-4")
    ...
    async def build_scout(self):
        for rf in self.units(ROBOTICSFACILITY).ready.noqueue:
            print(len(self.units(OBSERVER)), self.stime/3)
    ...
    async def build_gateway(self):
        pylons = self.units(PYLON).ready.noqueue
        if not pylons:
            return
        pylon = pylons.random
    ...
    async def attack_known_enemy_structure(self):
        if len(self.known_enemy_units) > 0 and len(self.units(NEXUS)):
